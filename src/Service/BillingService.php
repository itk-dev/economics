...
class BillingService
{
    ...
    public function exportInvoicesToSpreadsheet(array $invoiceIds): Spreadsheet
    {
        ...
        foreach ($invoiceIds as $invoiceId) {
            ...
            // 17. "Leverandør"
            if ($internal) {
                $sheet->setCellValueByColumnAndRow(17, $row, str_pad($this->receiverAccount, 10, '0', \STR_PAD_LEFT));
            }
            // 18. "EAN nr."
            ...
            foreach ($invoice->getInvoiceEntries() as $invoiceEntry) {
                ...
                // 2. "Materiale (vare)nr.
                $sheet->setCellValueByColumnAndRow(2, $row, str_pad($materialNumber->value, 18, '0', \STR_PAD_LEFT));
                // 3. "Beskrivelse"
