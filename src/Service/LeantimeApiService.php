...
class LeantimeApiService implements ProjectTrackerInterface {
  ...
  public function getProjectIssuesV2(string $projectId): IssueDataCollection {
    ...
    foreach ($issues as $issue) {
      $issueDataCollection->issueData->add(
        new IssueData(
          $issue->headline,
          $issue->status,
          $issue->id,
          ...
          null,
          null,
          ($issue->tags ?? $this->tagToId($issue->tags) ?? null),
          ($issue->tags ?? $issue->tags ?? null),
          ($issue->milestoneid && $issue->milestoneHeadline ? new ArrayCollection([new VersionData($issue->milestoneid, $issue->milestoneHeadline)]) : null),
          null,
          $issue->projectId
        )
      );
  ...
  {
    // $client = 
    $repository = $this->entityManager->getRepository(ClientRepository::class);
  ...
  public function getWorklogDataForProjectV2(string $projectId): WorklogDataCollection
  {
    ...
    $worklogs = $this->request(self::API_PATH_JSONRPC, "POST", "leantime.rpc.timesheets.getAll", ['invEmpl' => '-1', 'invComp' => '-1', 'paid' => '-1', 'projectId' => $projectId]);

    foreach ($worklogs as $worklog) {
      $worklogDataCollection->worklogData->add(
        new WorklogData(
          $worklog->id,
          $worklog->description ?? "",
          $worklog->userId,
          (int) ($worklog->hours * 60 * 60),
          new \DateTime($worklog->workDate),
          false,
          $worklog->ticketId
        )
        );
